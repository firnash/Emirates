...
export const AirportFilter = ({ label, name, data, selectedAirport, error, helperText }) => {
  ...
  return (
    <div className="airport-filter">
      <InputField error={error} helperText={helperText} handleFocus={focusHandler} handleChange={inputHandler} label={label} name={name} value={inputText} />
          ...
          <div className="airport-filter-dropdown">
            {
              filteredData.map((item, index) => {
                return <AirportFilterList handleItem={() => getSelectedCity(index)} key={index} city={item.city.longName} airport={item.longName} code={item.iataCode} />
              })
            }
          </div> : <></>
      }
